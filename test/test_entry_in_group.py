...
def test_add_entry_in_group(app, orm):
    if len(orm.get_groups_without_entries()) == 0:
        app.group.create_group(Group(name="test_group"))

    if len(orm.get_entries_not_in_any_group()) == 0:
        app.entry.create_entry(Entry(first_name="test_entry"))

    groups = orm.get_groups_without_entries()
    selected_group = random.choice(groups)

    entries = orm.get_entries_not_in_any_group()
...
def test_delete_entry_from_group(app, orm):
